...
use crate::{
    ...
    channel::Channel,
    pool::Pool,
    util::{as_bytes, BuilderExt, Unite},
};
...
impl Builder {
    ...
    pub fn pool_size(&mut self, amt: usize) -> &mut Self {
        assert!(
            self.pool_config.initial_pooled_contexts.unwrap_or(0) <= amt,
            "the provided pool size is less than initial_pooled_contexts"
        );
        self.pool_config.pool_size = amt;
        self
    }
    ...
    pub fn initial_pooled_contexts(&mut self, amt: usize) -> &mut Self {
        assert!(
            amt <= self.pool_config.pool_size,
            "the provided initial_pooled_contexts is greater than pool_size"
        );
        self.pool_config.initial_pooled_contexts = Some(amt);
        self
    }

    fn init_buf_size(&self) -> usize {
        BUFFER_HEADER_SIZE + *PAGE_SIZE * MAX_MAX_PAGES
    }

    #[allow(clippy::cognitive_complexity)]
    async fn try_init_session<I: ?Sized>(&self, io: &mut I) -> io::Result<Option<Session>>
    where
    ...
    pub async fn mount<'a>(
        ...
        let mut buf = vec![0u8; self.init_buf_size()];
        let session = loop {
            channel.read(&mut buf[..]).await?;
            match self
                .try_init_session(&mut Unite {
                    reader: &mut &buf[..],
                    writer: &mut channel,
                })
                .await?
            ...
            ($( $Variant:ident => $name:ident, )*) => {
                match op {
                    ...
                    OperationKind::Init { .. } => {
                        ...
                        channel.send_msg(header.unique, -libc::EIO, &[]).await?;
                    }
...
impl Server {
    ...
    {
        let sig = futures::future::pending::<()>();
        ...
        F: Filesystem + Send + 'static,
    {
        ...
        let mut pool = Pool::new(self.pool_config.pool_size);

        // Prepare the request contexts and store them into the pool.
        if let Some(amt) = self.pool_config.initial_pooled_contexts {
            for _ in 0..amt {
        ...
        F: Filesystem,
    {
        ...
        loop {
            ...
            self.session
                .process(&*fs, &mut buf, &mut self.channel)
                .await?;
        }
    }
    ...
    pub async fn notify_retrieve(&mut self, ino: u64, offset: u64, size: u32) -> io::Result<u64> {
        ...
        let unique = self.session.notify_unique.fetch_add(1, Ordering::SeqCst);
    ...
    F: Filesystem,
{
    ...
    async fn process(&mut self) -> io::Result<()> {
        self.session
            .process(&*self.fs, &mut self.read_buf, &mut self.writer)
            .await
    }
}
...
struct SessionOperation<'ctx, Op> {
    channel: &'ctx mut Channel,
...
impl<'ctx> SessionOperation<'ctx, Setattr<'ctx>> {
    #[inline(always)]
